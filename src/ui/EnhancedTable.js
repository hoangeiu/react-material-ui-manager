...
const EnhancedTableToolbar = (props) => {
  ...
  const [alert, setAleart] = useState({ open: false, color: "#FF3232", message: "Row deleted!" });
  ...
  const onDelete = () => {
    ...
    setSelected([]);
    setAleart({ ...alert, open: true });
  };

  const onUndo = () => {
    setSelected([]);
    setAleart({ ...alert, open: false });
    const newRows = [...rows];
    const redo = [...undo];
  ...
  const handleTotalFilter = (event) => {
    ...
    if (event.target.value !== "") {
      const newRows = [...rows];
      newRows.map((row) =>
        eval(`${event.target.value}${totalFilter === "=" ? "===" : totalFilter}${row.total.slice(1, row.total.length)}`)
          ? (row.search = true)
          : (row.search = false)
      );
  ...
  const filterChange = (operator) => {
    if (filterPrice !== "") {
      const newRows = [...rows];
      newRows.map((row) =>
        eval(`${filterPrice}${operator === "=" ? "===" : operator}${row.total.slice(1, row.total.length)}`)
          ? (row.search = true)
          : (row.search = false)
      );
          ...
          <TextField
            ...
            InputProps={{
              ...
              endAdornment: (
                <InputAdornment
                  onClick={() => {
                    setTotalFilter(totalFilter === ">" ? "<" : totalFilter === "<" ? "=" : ">");
                    filterChange(totalFilter === ">" ? "<" : totalFilter === "<" ? "=" : ">");
                  }}
                  position="end"
